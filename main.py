...
class Window(ctk.CTk):
    ...
    def save_parameters(self):
        ...
        bytes16_19 = struct.pack('<f', 0.05 if (self.mode.VenPW == '') else float(self.mode.VenPW))
        bytes20_21 = struct.pack('<H', 150 if (self.mode.VRP == '') else int(self.mode.VRP))
        bytes22_23 = struct.pack('<H', 150 if (self.mode.ARP == '') else int(self.mode.ARP))
        byte_24 = struct.pack('<B', 50 if (self.mode.MaxSensorRate == '') else int(self.mode.MaxSensorRate))
        byte_25 = struct.pack('<B', 10 if (self.mode.ReactionTime == '') else int(self.mode.ReactionTime))
        byte_26 = struct.pack('<B', 1 if (self.mode.ResponseFactor == '') else int(self.mode.ResponseFactor))
        byte_27 = struct.pack('<B', 2 if (self.mode.RecoveryTime == '') else int(self.mode.RecoveryTime))
