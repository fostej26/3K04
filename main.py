...
class Window(ctk.CTk):
    ...
    def init_login_page(self):
        ...
        self.logo_image = ImageTk.PhotoImage(logo_resized_image)  # Store the image in self to prevent garbage collection
        ...
        self.register_button = ctk.CTkButton(self.login_frame, text="Register", cursor="hand2", command=self.handle_register)
            ...
            for user in users:
                ...
                if username == user_data[0]:
                    if hashed_password == user_data[1]:
                        ...
                        self.message_label.configure(text="Incorrect password. Please try again.")
                        return
            ...
            with open("users.txt", "r") as f:
                ...
                for user in users:
                    if username == user.split()[0]:
                        self.message_label.configure(text="Username already registered. Please sign in or choose a new username.")
                        return
    ...
    def init_pacemaker_page(self):
        ...
        # Create a frame for connection status and place it in the grid
        ...
        # Add a label inside the connection status frame
        ...
        AOO_button = ctk.CTkRadioButton(pacemaker_modes_frame, text="AOO", font=("Helvetica", 12), variable=pacemaker_mode_var, value="AOO")
        AOO_button.grid(row=1, column=0, padx=5, pady=2, sticky="n")

        VOO_button = ctk.CTkRadioButton(pacemaker_modes_frame, text="VOO", font=("Helvetica", 12), variable=pacemaker_mode_var, value="VOO")
        VOO_button.grid(row=2, column=0, padx=5, pady=2, sticky="n")

        AAI_button = ctk.CTkRadioButton(pacemaker_modes_frame, text="AAI", font=("Helvetica", 12), variable=pacemaker_mode_var, value="AAI")
        AAI_button.grid(row=3, column=0, padx=5, pady=2, sticky="n")

        VVI_button = ctk.CTkRadioButton(pacemaker_modes_frame, text="VVI", font=("Helvetica", 12), variable=pacemaker_mode_var, value="VVI")
    ...
    def change_password_page(self):
        ...
        info_label = ctk.CTkLabel(self.change_password_frame)
        info_label.pack(pady=10)

        enter_info_button = ctk.CTkButton(self.change_password_frame, text="Change", command=lambda: self.enter_info(username.get(), newpassword.get(), info_label))
